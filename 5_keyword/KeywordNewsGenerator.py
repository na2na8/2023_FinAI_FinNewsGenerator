...
class KeywordNewsGenerator(pl.LightningModule) :
    ...
    def validation_epoch_end(self, outputs, state='valid') :
        ...
        # if not os.path.isfile(f'./final_outs/{self.tensorboard}_trgts.txt') :
        if self.current_epoch == 1 :
            with open(f'/home/nykim/finAI/0_numbers/with_spcecial_tokens/{self.tensorboard}_trgts.txt', 'a') as f :
                str_trgts = '\n\n'.join(self.trgts)
                f.write(str_trgts)
                f.close()
            
        with open(f'/home/nykim/finAI/0_numbers/with_spcecial_tokens/{self.tensorboard}_preds_{self.current_epoch}.txt', 'a') as f :
            str_preds = '\n\n'.join(self.preds)
            f.write(str_preds)
            f.close()
